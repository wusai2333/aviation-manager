...
<body>
	...
	<div id="book">
		<h2>接受订单</h2>
		<form action="receiver_book_ticket.jsp" name="form1" method="post" onsubmit="return on_submit()">
	...
	<%
	   ...
	   if(orderId!=null){
		   List<BookTicket> allTickets = MyControls.getAllBook();
		   for(int i=0;i<allTickets.size();i++){
			   if(allTickets.get(i).orderId.equals(orderId)){
				   //就是这个添加进去订票表中
				   boolean isDelete = MyControls.deleteOrder(orderId);
				   boolean result = MyControls.insertOrder(allTickets.get(i));
				   if(result){
		...
		<table id="tb">
			...
			<%
				...
				for (int i = 0; i < allTickets.size(); i++) {
					...
					out.print("<td width='110px'>" + allTickets.get(i).seat);
					out.print("</tr>");
